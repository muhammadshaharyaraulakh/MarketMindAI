...
export default function DataIngestion({ state, dispatch }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 text-left">
      ...
      <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 shadow-sm">
        ...
        {uploadSuccess && (
          <div className="border border-green-200 bg-green-50 rounded-xl p-8 flex flex-col items-center justify-center text-center animate-fadeIn">
            ...
            <h4 className="text-sm font-bold text-green-900 mb-1">✓ 1,240 rows ingested successfully</h4>
            <p className="text-xs font-semibold text-green-700">Your dataset is now available for AI analysis.</p>
          </div>
        )}
      </div>
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex justify-end gap-3">
                        <button className="text-[11px] font-bold text-[#FF2D20] hover:text-[#E5261A] cursor-pointer">
                          {upload.status === 'Failed' ? 'Retry' : 'View'}
                        </button>
      ...
      {/* SECTION 3 - DEMO DATA SEEDER */}
      <div id="demo-section" className="bg-white border border-[#E2E8F0] rounded-2xl p-6 shadow-sm">
        <h3 className="text-sm font-bold text-[#0F172A] font-mona mb-1">Demo Dataset Library</h3>
        <p className="text-xs font-semibold text-[#94A3B8] mb-6">Load pre-built campaign datasets to explore all features without uploading real data.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={pack.id} className="border border-[#E2E8F0] rounded-xl p-5 flex flex-col justify-between hover:border-[#CBD5E1] transition-colors relative">
              {pack.tag && (
                <span className="absolute -top-2.5 right-4 bg-[#0F172A] text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                  {pack.tag}
                </span>
              )}
              <div className="mb-4">
                <h4 className="text-sm font-bold text-[#0F172A] font-mona">{pack.title}</h4>
                <p className="text-[11px] font-semibold text-[#475569] mt-1">{pack.sub}</p>
              </div>
              <button
                onClick={() => handleLoadDemo(pack.id, pack.rows, pack.platform)}
                disabled={demoLoadingId === pack.id || demoSuccessId === pack.id}
                className="w-full bg-[#FF2D20] hover:bg-[#E5261A] text-white text-xs font-bold py-2 rounded-lg cursor-pointer transition-colors shadow-sm disabled:opacity-90 flex items-center justify-center gap-1"
              >
