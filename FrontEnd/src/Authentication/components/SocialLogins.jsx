...
export default function SocialLogins({ onSuccess, mode = 'login' }) {
  ...
  const handleProviderClick = (providerName) => {
    ...
    setTimeout(() => {
      ...
      if (mode === 'signup') {
        let subtitle = ''
        if (providerName === 'Google') subtitle = 'You successfully logged in using Google account verification. Welcome back.'
        if (providerName === 'GitHub') subtitle = 'You successfully logged in using GitHub integration.'
        if (providerName === 'Apple') subtitle = 'You successfully registered via secure Apple ID authentication.'
        
        onSuccess({
          title: 'Workspace Initialized',
          subtitle,
          actionText: providerName === 'Apple' ? 'Launch Panel' : (providerName === 'Google' ? 'Proceed to Panel' : 'Enter Space'),
          actionView: 'dashboard'
        })
      } else {
        // Login mode
        onSuccess() // no context needed for login, triggers onClose({ loggedIn: true })
      }
    }, 1000)
  }
